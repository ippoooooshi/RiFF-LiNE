...
**0.27 実装フェーズ第8回 — 作業パッケージ8「画面群・ナビゲーション」着手（2026-09-09）。Phase 1（PC版MVP）実装フェーズ完了**：[[15_development_process.md#4.1]]実施順8に従い`feature/screens-navigation`ブランチで着手。読み合わせ（[[03_screens_ui_pc.md]]全節・[[14_visual_design_system.md]]全節・[[13_design_decision_points.md]]B10/B19/B20/B34/C3/C12・[[../detailed_design/screens-navigation.md]]・[[../detailed_design/00_reference.md]]§2/§3/§4/§5/§8.1 G1/G22/G23/G24/§9）の結果、着手をブロックする未解消ギャップなし（G1 は WP4〜7 と同じく詳細設計書本体を埋め戻さず as-built を[[../detailed_design/00_reference.md#3.8]]へ反映）。`packages/core/src/ui`に`AppPreferencesService`（`preferences.json`、`settings.json` と分離、未知キー除去・型不一致既定値化・数値クランプ）／`TagStore`（`tags.json` CRUD、50件で`TAG-001`＋`null`）／`ThumbnailGenerator`（先頭1段 SVG→`base64-png`、`wrapSave` 非破壊フック、B7・G6）／`NotificationUIBinder`＋`ScoreHighlightBinder`（`NotificationCenter`購読→toast/highlight/modal 振り分け→`ScoreRenderHost` ハイライト）／`ToolbarViewModel`＋`StatusBarViewModel`（編集ウィンドウごと）／`MenuBarController`（`MenuItemDescriptor[]`、B19 でエクスポート/印刷既定無効）／`KeyboardShortcutRouter`（テキスト入力欄優先）／`PartColorOverlay`（G24 解消、`boundsLookup` 幾何オーバーレイの DOM/CSS 生成）／`PlaybackPreferencesAdapter`（`AppPreferencesService`→`PlaybackPreferencesSource`、9.10節の実体化）／`uiErrorCodes`（`TAG-001`/`SONG-001`/`SONG-002`）を実装。`apps/desktop/src/main/WindowManager.ts`（`WindowAdapter` 実装、`focusExistingWindow` の複合条件 C2、クローズ時 `flushAutoSave` 待ち）と `window:*` IPC を追加。`apps/desktop/src/renderer/screens`に 03章画面インベントリ15件（#8 除く）の React シェル、`App.tsx` を `#songlist`／`#edit/<songId>` ルーティング＋DI 組み立てに置換。`ScoreRenderHost` へ `showErrorHighlight`／`clearErrorHighlight`／`getPartRegions` を非破壊追加。実装レベル判断1件を確定：React 画面コンポーネントを `packages/core/src/ui`（[[../rules/ui.rule.md]]）ではなく `apps/desktop/src/renderer/screens` に置き、フレームワーク非依存ロジック（ViewModel/Binder/Service）のみコアに残す分割（[[13_design_decision_points.md#3]]B36、[[../detailed_design/00_reference.md#9]]§9.25）。G24 は解消（`PartColorOverlay`＋`getPartRegions`）、G23 は本パッケージ担当で §9.0 に P1〜P7 の結果を記録（自動テスト＋ヘッドレスで代替検証済み、実 UI 目視は本人環境待ち）。**2026-09-09 独立レビュー（sdlc-impl-review）差し戻しを是正**：(B-1) `ThumbnailGenerator` の既定ラスタライザが SVG ソースを base64 化して `base64-png` と偽っていた自己矛盾を実 PNG 変換（`<img>`→`canvas`）へ是正し `defaultRasterizer`/`rasterizeEnv` に直接 UT を追加、(B-2) クローズ時自動保存 flush の投げっぱなし＋文字列直書きを `AutoSaveFlushBridge`（トークン付きハンドシェイク＋タイムアウト、`WINDOW_CHANNELS` 定数）へ是正、(#1・#2) 新規曲作成フローを `songActions.createSongAndOpen` へ実配線し `SONG-001`/`SONG-002` の発火経路を通した、(#5) `WindowManager.finalizeClose` の二重 dispose を管理表除去順で防止、(#3・#4) `§9.26`→`§9.25` 参照ずれと `06_file_io_persistence.md` §3 の `preferences.json` 未記載を是正。**さらに本人実機で P2-a が FAIL（編集ウィンドウ全白）→ 2回目の是正**：(B-3) `App.tsx` `EditWindow` の bootstrap 初期化順バグ（`host.initialize()` 前に `ViewModeController` を構築し `applyViewMode` → `NOT_INITIALIZED` 同期 throw → React がツリー unmount）を、初期化先行＋try/catch フォールバック＋`ErrorBoundary` で是正。(B-4) 無試験だった renderer bootstrap（`App.tsx` の DI 配線）を jsdom＋`.tsx` の新規 vitest プロジェクト `desktop-renderer`（`appBootstrap.test.tsx`）で固定。(G25) 15画面シェルは 14章デザインシステム未到達＝B36 シェル方針による意図的な最小実装（本人合意、[[../detailed_design/00_reference.md#8.1]] G25）。`pnpm typecheck`／`pnpm lint`／`pnpm test`（765 pass / 1 skip、パッケージ8 UT/IT +127）／`pnpm build`／`pnpm format` 緑。**これで Phase 1（PC版MVP）の実装フェーズが全8パッケージ完了（実 UI 目視 P1/P2-a は本人環境で再試験待ち）。**

**0.26 実装フェーズ第7回 — 作業パッケージ7「再生エンジン統合」着手（2026-09-09）**：[[15_development_process.md#4.1]]実施順7に従い`feature/playback-integration`ブランチで着手。読み合わせ（[[05_playback_audio.md]]全節・[[09_nonfunctional.md]]§1・[[../detailed_design/playback-integration.md]]・[[../detailed_design/00_reference.md]]§3.7/§2/§4/§5/§8.1 G22・[[13_design_decision_points.md]]A4/B14/B17/B18/C1/C2）の結果、未決着のカテゴリB／C事項0件・着手をブロックする未解消ギャップなし（G1 は WP4〜6 と同じく詳細設計書本体を埋め戻さず as-built を[[../detailed_design/00_reference.md#3.7]]へ反映）。`packages/core/src/playback`に`PlaybackService`（再生／停止／シーク／区間・セクションループ〈境界で開始小節へ再シーク〉／ソロ再生指定／テンポ倍率／`preWarm`／`resolvePlaybackPitch`、編集ウィンドウ単位）／`PlaybackSyncController`（`onCommandApplied`購読で再生中のみ dirty 化、小節境界＝ループ境界含む単一ルールで`AudioSyncStrategy`フラッシュ）／`PlaybackMixerBinder`（通知のたびに全チャンネルへ volume/pan/solo＋ソロ優先の実効 mute を一律再送）／`AudioSyncStrategy`（`PartialReloadStrategy`／`PauseResumeStrategy`、A4 未解決のため既定＝`PauseResumeStrategy`、設定切替のみで入替可）／`MetronomeService`／`CountInController`（C2、1/2小節倍率）／`TapTempoController`（直近3〜4タップ平均→`SetTempoCommand`発行）／`PlaybackCursorFollow`を実装。カポ運指→実音変換式を`domain/pitch.ts`の共有純粋関数`computeRealMidiPitch`（B18）へ抽出し、`ChordDetectionService`もこれへ委譲（単一の真実源、[[../detailed_design/00_reference.md#2]]）。2件の実装レベル構造判断を確定：(1) 生`AlphaSynth`は`PlaybackSynth`インターフェース（唯一の薄い窓口）に閉じ込め、その実体化と`preWarm`の起動シーケンス挿入配線は bootstrap（パッケージ8）／Phase 1 実機検証へ委譲（[[../detailed_design/playback-integration.md#10]]§10.1）。(2) [[../detailed_design/view-modes.md#9]]が約束していた「表示範囲更新API」の実体が無かったため、`ViewModeController`へ`isBarVisible`／`revealBar`を非破壊追加（[[../detailed_design/00_reference.md#3.6]]・§4）。新規エラーコードなし。DoD 基準5（手動シナリオ）は 0.24 と同じくパッケージ8へ繰り越し（G23 のパッケージ7分を追記）。`pnpm typecheck`／`pnpm lint`／`pnpm test`（637 pass / 1 skip、`playback` UT/IT 113・`computeRealMidiPitch` UT 10・`ViewModeController`拡張 UT 5）／`pnpm build` 緑。

**0.25 実装フェーズ第6回 — 作業パッケージ6「表示モード」着手（2026-09-09）**：[[15_development_process.md#4.1]]実施順6に従い`feature/view-modes`ブランチで着手。読み合わせ（[[../tab_app_requirements.md]]§4.1/§4.5・[[03_screens_ui_pc.md]]§5・[[09_nonfunctional.md]]§1/§2・[[../detailed_design/view-modes.md]]・[[../detailed_design/00_reference.md]]§3.6〜§8.1・[[13_design_decision_points.md]]A9/B11/B16）の結果、未決着のカテゴリB／C事項0件・着手をブロックする未解消ギャップなし（G1 は WP4/5 と同じく詳細設計書本体を埋め戻さず as-built を[[../detailed_design/00_reference.md#3.6]]へ反映）。`packages/core/src/viewmodes`に`ViewModeController`（focus/scroll/score の切替、モード切替時カーソル不変、フォーカスビューの表示範囲追従＝範囲外に出た時のみ更新、パート追従）／`ZoomController`（モードごとに独立したズーム％保持、B16。`[25,400]`クランプ、`AppPreferencesService`由来初期値の注入口、ステータスバー通知コールバック）を実装。`ScoreRenderHost`へ`applyViewMode(request)`／`applyZoom(scale)`を非破壊追加（alphaTab `settings.display.{scale,layoutMode,startBar,barCount}`＋`renderTracks`）。1件の実装レベル構造判断を確定：[[../detailed_design/view-modes.md#4.3]]の`data-track-index`属性方式は alphaTab 1.8.4 の SVG 出力（フラット element tree、トラック単位の DOM 要素なし）に付与先が無く実装不能のため、スコア表示のパート識別を`boundsLookup`由来の幾何オーバーレイ方式へ変更し、色オーバーレイの DOM/CSS 生成本体と実 UI 目視は パッケージ8へ繰り越し（[[13_design_decision_points.md#3]]B34、[[../detailed_design/00_reference.md#8.1]] G24、9.23節）。DoD 基準5（手動シナリオ）は 0.24 と同じくパッケージ8へ繰り越し（G23 のパッケージ6分を追記）。`pnpm typecheck`／`pnpm lint`／`pnpm test`（524 pass / 1 skip、`viewmodes` UT 39・`ScoreRenderHost` 拡張 UT 11・IT-VIEW 5）／`pnpm build` 緑。

**0.24 パッケージ1〜5 の DoD 基準5（手動シナリオ）をパッケージ8へ繰り越し（2026-09-08、本人の指示）**：パッケージ1〜5 はいずれも操作対象の実 UI（曲一覧・編集画面・パートリスト／ミキサー・チューニング設定パネル・設定ダイアログ等）が[[../detailed_design/screens-navigation.md]]（パッケージ8）で実装されるため、[[15_development_process.md#7]]基準5（[[11_test_strategy.md#6]]システムテスト観点の手動シナリオ）を本来の操作フローとして完了できていない。各パッケージは代替手段（CDP ヘッドレス検証・通し結合テスト・複数編集ウィンドウ分離テスト）でシステムテスト観点を暫定的に担保して `main` へマージ済み。実 UI での手動シナリオ（P1〜P5）を[[../detailed_design/00_reference.md#8.1]] G23 として登録し、パッケージ8着手時のチェックリスト（[[../detailed_design/screens-navigation.md#9]] §9.0）＋ DoD（[[../detailed_design/screens-navigation.md#8]]）に組み込んだ。各パッケージの詳細設計 DoD 節（[[../detailed_design/web-core-foundation.md#8]]／[[../detailed_design/data-model-persistence.md#10]]／[[../detailed_design/error-logging-foundation.md#8]]／[[../detailed_design/editing-core.md#13]]／[[../detailed_design/part-tuning-management.md#8]]）にも繰り越しの旨を明記。

**0.23 実装フェーズ第5回 — 作業パッケージ5「パート・チューニング管理」着手（2026-09-08）**：[[15_development_process.md#4.1]]実施順5に従い`feature/part-tuning-management`ブランチで着手。読み合わせ（[[02_data_model.md]]§3.2/§3.7・[[03_screens_ui_pc.md]]§5/§10・[[05_playback_audio.md]]§7・[[../detailed_design/part-tuning-management.md]]・[[../detailed_design/00_reference.md]]§3.5〜§8.1・[[13_design_decision_points.md]]B14/B15/B20/B27）の結果、未決着のカテゴリB／C事項0件。G1 は WP4 と同じく詳細設計書本体を埋め戻さず as-built を[[../detailed_design/00_reference.md#3.5]]へ反映。`packages/core/src/parts`に`PartManagementService`／`TuningPresetService`＋`TuningPresetStore`（`tuning-presets.json`、組み込み6種、B27 論理削除＋7日/20件パージ）／`PartColorAllocator`（ステートレス8色）／`PartValidationService`（`ValidationService`のサブクラスとして非破壊拡張、`EDIT-005`〜`007`）／コマンド11種（`SetPart{Volume,Pan,Solo,Mute,Color}`・`SetCapoFret`・`AddPart`・`RemovePart`・`ReorderParts`・`ApplyTuningPreset`・`SetCustomTuning`）を実装。`ApplyTuningPresetCommand`は B15 の弦数同期（減少時 Note 破棄＋`EDIT-006`）を含む。既知の限定事項：プリセット出自の`presetId`を保存する Part フィールドが alphaTab モデルに無いため、当面は適用時のプリセット名を`staff.stringTuning.name`に記録するに留め、id レベルの provenance は data-model 拡張時へ申し送り（G22）。`pnpm typecheck`／`pnpm lint`／`pnpm test`（469 pass / 1 skip、パート管理 UT 58・IT 4）／`pnpm build` 緑。

**0.22 実装フェーズ第4回 — 作業パッケージ4「タブ譜編集コア」着手（2026-09-08、XL）**：[[15_development_process.md#4.1]]実施順4に従い`feature/editing-core`ブランチで着手。読み合わせ（[[04_editing_core.md]]・[[02_data_model.md]]・[[../detailed_design/editing-core.md]]・[[../detailed_design/00_reference.md]]§3.4〜§8.1・[[13_design_decision_points.md]]A1/A8/B2〜B5/B20/C11/C13）の結果、未決着のカテゴリB／C事項0件。既知ギャップG1（詳細設計書がパッケージ4〜8分は責務レベル止まり）は本人の指示により埋め戻さず、責務レベルの詳細設計を正として実装し as-built シグネチャを[[../detailed_design/00_reference.md#3.4]]へ反映（G1 は詳細設計書本体については未解消のまま）。`packages/core/src/editing`に`CursorController`／`EditingService`／`Command`I/F＋`CommandHistory`（再描画一元化・80MB予算/200件下限/エビクション/`onCommandApplied`/結合、C11）／`CompositeCommand`／具象コマンド19種／`ValidationService`（`EDIT-001`〜`004`）／`ChordDetectionService`／`ClipboardService`を実装。`EDIT-009`（ペースト時弦数不足のWarning）を実装時に採番。実装レベルの構造判断を確定：コマンドは alphaTab `Score.finish()` を呼ばず派生計算は `CommandHistory`→`ScoreRenderHost.render()` の再描画経路に一任する（[[13_design_decision_points.md#3]]B33、[[../detailed_design/00_reference.md#3.4]]・9.21節）。`InsertBar`/`DeleteBar`/`Paste` は `CompositeCommand` ではなく全パート同期の単一コマンドとして実装。`pnpm typecheck`／`pnpm lint`／`pnpm test`（396 pass / 1 skip、編集コア UT 121・IT 4）／`pnpm build` 緑。
