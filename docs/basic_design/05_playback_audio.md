...
- ソロ指定パートが1つ以上ある場合、AlphaSynth側の対応するトラックチャンネルのみをアクティブにし、それ以外は内部的にミュート扱いとする。**同一トラックに明示ミュート（`Part.mute`）とソロ（`Part.solo`）が両方指定された場合は、明示ミュートを優先して無音とする**（明示ミュートは「このトラックは鳴らさない」という絶対操作であり、ソロは「ソロ対象以外を内部ミュートする」機能としてユーザーの明示ミュートを上書きしない。一般的なDAWの挙動に揃える。[[13_design_decision_points.md#3]]B35、[[../detailed_design/playback-integration.md#4.3]]）。
