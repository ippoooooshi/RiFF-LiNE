...
`pnpm typecheck`／`pnpm lint`／`pnpm test`（468 pass / 1 skip、パート管理 UT 57・IT 4）／`pnpm build` 緑。全コマンドの execute/undo 対称性を JSON 往復で C2 検証、`ApplyTuningPresetCommand` の弦数増加/減少/同数、ミキサーのドラッグ結合、`PartColorAllocator` の8色境界・reserved 除外、`purgeExpired` の7日/20件境界、複数編集ウィンドウでのパート操作分離を含む。
