...
| `ValidationService` | 4（タブ譜編集コア、ノート配置・小節数・メモ文字数検証） | **2026-09-08 実装済み**：`validateNotePlacement`／`validateBarInsertion`／`validateMemoText`（3.4節、`EDIT-001`〜`004`）。パッケージ5がパート数上限(`EDIT-005`)・チューニングプリセット弦数同期(`EDIT-006`)・カポ範囲(`EDIT-007`)を非破壊追加する予定 |
