...
**2026-09-07 実装ステータス（`feature/data-model-persistence`）**：基準1〜3・6 は充足（`pnpm typecheck`／`pnpm lint`／`pnpm test`〈195 pass / 1 skip〉／`pnpm build` が緑）。実装は 3 節のクラス構成・シグネチャに沿い、3.3.1 節（B31）・9.7〜9.8 節の実装レベル判断を詳細設計へ反映済み。単体テスト（8 節観点）：`SchemaMigrator`／`ChecksumUtil`（sha256 は既知ベクタ）／`StorageConfigService.validateMirrorConfig`（C2）／`TrashService.purgeExpired`（29/30/31 日境界）／`AutoSaveScheduler`（デバウンス・最大遅延・リトライ、fake timers）を含む全メソッドの C0/C1。結合テスト：`apps/desktop/src/main/*.test.ts` で `fs:*`／`fs:*At`／`appconfig:*` の IPC 往復と実 I/O（OS 一時ディレクトリ）。クラウドフォルダでの結合テストは A5 実機検証待ち（基準3 の但し書きどおり）。**基準4（セルフレビュー）は実施・是正済み。基準5（手動シナリオ）**：曲の新規作成→自動保存→再起動復元・ゴミ箱・保存先切替・`LocalBackupService`・終了時ミラー待ちの通し確認には 3.3.1 節のブートストラップ合成点（DI 組み立て）が必要で、これはパッケージ8実装のため実 UI での通し実行ができていない。IPC 経路（`fs:*At`／`appconfig:*`）の疎通は結合テストで担保したうえで `main` へマージ済みとし、実 UI での手動シナリオは[[00_reference.md#8.1]] G23 としてパッケージ8着手時の繰り越し項目に登録した（[[screens-navigation.md#9]] P2-a〜P2-c）。
